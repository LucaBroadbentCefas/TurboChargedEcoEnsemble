...
// ADD back the helper functions in functions{}
...
transformed parameters{
  ...
  */
  ...
  // ADD back to transformed parameters{}
...
for (i in 1:(MM + M + 2)){
  ...
  latent[1][beg_in[i]:end_in[i]] =
    x_hat[beg_in[i]:end_in[i]]
    + cholesky_decompose(SIGMA_init[beg_in[i]:end_in[i], beg_in[i]:end_in[i]])
      * latent_raw[1][beg_in[i]:end_in[i]];
  for (t in 2:time){
    latent[t][beg_in[i]:end_in[i]] =
      AR_params[beg_in[i]:end_in[i]] .* latent[t-1][beg_in[i]:end_in[i]]
      + ch_SIGMA[i] * latent_raw[t][beg_in[i]:end_in[i]];
  }
}
