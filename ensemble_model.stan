...
transformed data{
  ...
  bigM = all_eigenvectors_cov' * bigM;
    ...
    array[M + 2] int beg_in;
  array[M + 2] int end_in;

  beg_in[1] = 1;          end_in[1] = N;          // observations
  beg_in[2] = N + 1;      end_in[2] = 2 * N;      // consensus
  for (i in 1:M) {
    beg_in[i + 2] = (i + 1) * N + 1;
    end_in[i + 2] = (i + 2) * N;
  }
...
parameters{
  ...
  array[time] vector[(M + 2) * N] latent_raw;
...
transformed parameters{
  ...
  */
  ...
  for (i in 1:M){
    ...
    array[time] vector[(M + 2) * N] latent;
  array[M + 2] matrix[N, N] ch_SIGMA;

  for (b in 1:(M + 2)) {
    ch_SIGMA[b] = cholesky_decompose(SIGMA[beg_in[b]:end_in[b], beg_in[b]:end_in[b]]);

    // t = 1 draw with initial covariance
    latent[1][beg_in[b]:end_in[b]] =
      x_hat[beg_in[b]:end_in[b]]
      + cholesky_decompose(
          SIGMA_init[beg_in[b]:end_in[b], beg_in[b]:end_in[b]]
        ) * latent_raw[1][beg_in[b]:end_in[b]];

    // t >= 2 AR(1) recursion with innovation covariance
    for (t in 2:time) {
      latent[t][beg_in[b]:end_in[b]] =
        AR_params[beg_in[b]:end_in[b]] .* latent[t - 1][beg_in[b]:end_in[b]]
        + ch_SIGMA[b] * latent_raw[t][beg_in[b]:end_in[b]];
    }
  }
...
model{
  ...
  */
   ...
   */
for (t in 1:time) {
  latent_raw[t] ~ std_normal(); // prior for latent innovations
}

{
  ...
  for (t in 1:time) {
    for (j in 1:cols(new_data)) {
      if (observation_available[t, j] == 1.0) {
        real est = dot_product( (latent[t] + lt_discrepancies)', bigM[j,] );
        new_data[t, j] ~ normal(est, R[j]);
      }
    }
  }
}
