functions{
  ...
  int which_fun_subset(int i, int j, array[,] int mod_to_dri){
    ...
    int[] which_fun_ms(int i, int j, array[] int sim_num, array[] int esm_num, int model_num_species, int plus_N){
    array[model_num_species] int ret;
	int x = 0;
	int num = 0;
	while(num < model_num_species){
	  x += 1;
	  if (sim_num[x] == i){
	    if (esm_num[x] == j){
		  num += 1;
 ...
 int[] which_fun_ms_i(int i, array[] int sim_num, int model_num_species, int plus_N){
    array[model_num_species] int ret;
	int x = 0;
	int num = 0;
	while(num < model_num_species){
...
array[M+MM+2] int beg_in;
  array[M+MM+2] int end_in;
  ...
  for(i in 1:MM){
    beg_in[i + 2 + M] =(i+ M + 1) * N + 1;
	end_in[i + 2 + M] = (i + M +2)*N ;
  }
...
parameters{
   ...
   */
	...
	array[time] vector[(MM + M+2) * N] latent_raw;
...
transformed parameters{
  ...
  */
  ...
  ///// explicitly describe latent variables
  array[time] vector[(MM + M + 2) * N] latent;

  array[MM + M + 2] matrix[N, N] ch_SIGMA;

  for(i in 1:(MM + M + 2)){
    ch_SIGMA[i]=cholesky_decompose(SIGMA[beg_in[i]:end_in[i],beg_in[i]:end_in[i]]);
	latent[1][ beg_in[i]:end_in[i]] = x_hat[beg_in[i]:end_in[i]] + cholesky_decompose(SIGMA_init[beg_in[i]:end_in[i],beg_in[i]:end_in[i]]) * latent_raw[1][beg_in[i]:end_in[i]];
    for (t in 2:time){
      latent[t][beg_in[i]:end_in[i]] = AR_params[beg_in[i]:end_in[i]] .* latent[t-1][beg_in[i]:end_in[i]] + ch_SIGMA[i] * latent_raw[t][beg_in[i]:end_in[i]];
    }
  }
    ...
    if (form_prior_ind_st == 3){
      ...
      for (k in 1:(N-1)){
		...
		/// MS to edit to include the new prior -- see conjugate prior wikipedia
		      target += beta_conj_prior(prior_ind_st_cor_hierarchical_beta_params[k, l],
		      prior_ind_st_cor_hierarchical_beta_params[l, k],
		      prior_ind_st_cor_hierarchical_beta_hyper_params[1],
		      prior_ind_st_cor_hierarchical_beta_hyper_params[2],
		      prior_ind_st_cor_hierarchical_beta_hyper_params[3]);
        }
      }
    }
...
for (i in 1:time){
    latent_raw[i] ~ std_normal();
  }

  vector[num_elements(all_eigenvalues_cov)] R = sqrt(all_eigenvalues_cov);

  for (i in 1:time){
    for (j in 1:cols(new_data)){
	  if (observation_available[i,j]==1.0){
	    real est = dot_product((latent[i] + lt_discrepancies)' ,  bigM[j,]);
		new_data[i,j] ~ normal(est,(R[j]));
	  }
	}
  }
